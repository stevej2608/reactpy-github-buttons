...
// src/js/node_modules/object-assign/index.js
...
// src/js/node_modules/react/cjs/react.development.js
...
// src/js/node_modules/react/index.js
...
// src/js/node_modules/scheduler/cjs/scheduler.development.js
...
// src/js/node_modules/scheduler/index.js
...
// src/js/node_modules/scheduler/cjs/scheduler-tracing.development.js
...
// src/js/node_modules/scheduler/tracing.js
var require_tracing = __commonJS((exports, module) => {
  var scheduler_tracing_development = __toESM(require_scheduler_tracing_development());
  if (false) {} else {
    module.exports = scheduler_tracing_development;
  }
});

// src/js/node_modules/react-dom/cjs/react-dom.development.js
...
// src/js/node_modules/react-dom/index.js
...
var forEach = (obj, func) => {
  for (let i = 0, len = obj.length;i < len; i++) {
    func(obj[i]);
  }
};
var createElementInDocument = (document3) => {
  return (tag, props, children) => {
    const el = document3.createElement(tag);
    if (props != null) {
...
var onEvent = (target, eventName, func) => {
...
var offEvent = (target, eventName, func) => {
...
var onceEvent = (target, eventName, func) => {
...
var onceReadyStateChange = (target, regex, func) => {
...
var parseOptions = (anchor) => {
  const options = {
    href: anchor.href,
    title: anchor.title,
    "aria-label": anchor.getAttribute("aria-label")
  };
  forEach(["icon", "color-scheme", "text", "size", "show-count"], (option) => {
...
var stringify = (obj, sep, eq, encodeURIComponent) => {
  if (sep == null) {
    sep = "&";
  }
  if (eq == null) {
...
var parse = (str, sep, eq, decodeURIComponent) => {
  if (sep == null) {
    sep = "&";
  }
  if (eq == null) {
...
var getColorSchemeMediaQuery = (systemColorScheme, widgetColorScheme) => {
...
var getColorScheme = (declarations) => {
  ...
  const colorSchemes = parse(declarations, ";", ":", (str) => {
    return str.replace(/^[ \t\n\f\r]+|[ \t\n\f\r]+$/g, "");
  });
...
var octicon = (icon, height) => {
  icon = toLowerCase(icon).replace(/^octicon-/, "");
  if (!hasOwnProperty(data, icon)) {
    icon = "mark-github";
  }
  const defaultHeight = height >= 24 && 24 in data[icon].heights ? 24 : 16;
...
var fetch = function(url, func) {
  ...
  const callback = dispatchOnce(function(error, data2) {
    delete queues[url];
    let f;
    while (f = queue.shift()) {
      f(error || false, data2);
...
var render$1 = function(root, options, func) {
  const createElement2 = createElementInDocument(root.ownerDocument);
  const style = root.appendChild(createElement2("style", {
    type: "text/css"
  }));
  const cssText = buttonsCssText + getColorScheme(options["data-color-scheme"]);
  if (style.styleSheet) {
  ...
  const widget = root.appendChild(createElement2("div", {
  ...
  fetch.call(this, apiBaseURL + api, (error, json) => {
    if (!error) {
      const data2 = json[property];
      widget.appendChild(createElement2("a", {
        className: "social-count",
        href: json.html_url + href,
        rel: "noopener",
        target: "_blank",
        "aria-label": data2 + " " + property.replace(/_count$/, "").replace("_", " ").slice(0, data2 < 2 ? -1 : undefined) + " on GitHub"
      }, [
...
var render = (options, func) => {
  ...
  if (useShadowDOM) {
    ...
    render$1(host.attachShadow({ mode: "open" }), parsedOptions, () => {
      func(host);
    });
  } else {
    const iframe = createElement("iframe", {
      src: "javascript:0",
      title: parsedOptions.title || undefined,
    ...
    const callback = () => {
      ...
      render$1.call(contentWindow, body, parsedOptions, (widget) => {
        const size = get(widget);
        iframe.parentNode.removeChild(iframe);
        onceEvent(iframe, "load", () => {
          set(iframe, size);
        });
        iframe.src = iframeURL + "#" + (iframe.name = stringify(parsedOptions));
        func(iframe);
      });
    };
  ...
  return {
    create: (type, props, children) => {
      if (node.childElementCount) {
        ...
        if (firstChild) {
          import_react_dom.default.unmountComponentAtNode(firstChild);
        }
      }
      return import_react.default.createElement(type, props, ...children);
    },
    render: (element) => {
      return import_react_dom.default.render(element, node);
    },
    unmount: () => {
      return import_react_dom.default.unmountComponentAtNode(node);
    }
  };
}
